...
Function that takes randomly sampled df, fits modes and stores estimates of model.
```{r}
...
corEvol <- function(df, n.min=10, item_nr=9) {
  ...
  res <- matrix(NA, nrow=length(seq(n.min*item_nr, nrow(df), by= item_nr)), ncol=4)
  ...
  for (i in seq(n.min*item_nr, nrow(df), by=item_nr)) {
    ...
    res[count, 1:4] <- c(n=i, estimate$coefficients[1], estimate$coefficients[2], estimate$coefficients[3])
    

  }
  colnames(res) <- c("item_nr", rownames(estimate$coefficients)[1], rownames(estimate$coefficients)[2],rownames(estimate$coefficients)[3]) 	
  return(res)	
}
```
...
n.max <- 300		# maximum number of participants per sample
